# Personal systemd *user* units — backup and log-sync jobs.
#
# NOTE: the units with `enable = false` are deliberately still declared. A
# disabled unit is materialised as a symlink to /dev/null, so deleting one
# instead of disabling it changes the generated unit directory.
{ pkgs, ... }:

{
  systemd.user.services.update-sky =
    let
      ...
      script = pkgs.writeScript "skycache-update.sh" ''
        ...
        mapcrafter -c ${mapcrafterConfig} -j 24
  ...
  systemd.user.services.update-logs = {
